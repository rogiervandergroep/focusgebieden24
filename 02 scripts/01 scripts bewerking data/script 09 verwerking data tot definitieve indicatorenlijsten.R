...
range <- data_def2 |>
  select(measure, jaar_min, jaar_max, starts_with("besch_")) |>
  distinct(measure, .keep_all = T)

# to do: indciatoren tabel inlezen
tabel_ind_def <- tabel_ind |>
  left_join(range, by = "measure")
...
tabel_ind_def2 <- tabel_ind_def |>
  mutate(
    ind_onderdeel_van = case_when(
      (mpzo == TRUE &
        aanpak_noord == TRUE &
        samen_nw == TRUE) ~ '1. MPZO, Aanpak Noord, Samen Nieuw-West',

      (mpzo == TRUE &
        aanpak_noord == TRUE &
        samen_nw == FALSE) ~ '2. MPZO, Aanpak Noord',
      (mpzo == TRUE &
        aanpak_noord == FALSE &
        samen_nw == TRUE) ~ '3. MPZO, Samen Nieuw-West',

      (mpzo == FALSE &
        aanpak_noord == TRUE &
        samen_nw == TRUE) ~ '4. Aanpak Noord, Samen Nieuw-West',

      (mpzo == TRUE & aanpak_noord == FALSE & samen_nw == FALSE) ~ '5. MPZO',
      (mpzo == FALSE &
        aanpak_noord == TRUE &
        samen_nw == FALSE) ~ '6. Aanpak Noord',
      (mpzo == FALSE &
        aanpak_noord == FALSE &
        samen_nw == TRUE) ~ '7. Samen Nieuw-West',
      (mpzo == FALSE & aanpak_noord == FALSE & samen_nw == FALSE) ~ '8. basis'
    )
  ) |>

  mutate(
    ind_onderdeel_van = factor(ind_onderdeel_van, levels = onderdeel_van_levels)
  )
...
write.xlsx(
  tabel_ind_def2,
  "01 indicatoren/Totaaloverzicht focusgebieden Amsterdam DEF 2026.xlsx",
  withFilter = T,
  overwrite = T,
  widths = 30
)
...
Indicatorenoverzicht <- tibble(
  "Indicatorenoverzicht" = c(
    "",
  ...
  mutate(
    Indicatorenoverzicht = case_when(
      Indicatorenoverzicht ==
        'ind_MPZO' ~ 'ind_MPZO lijst met indicatoren voor Masterplan Zuidoost',
      Indicatorenoverzicht ==
