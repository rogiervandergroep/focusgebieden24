...
### van enkele measuren wordt het echte gemiddelde uitgerekend op basis van de echte aantallen:
...
som_df <- BBGA_data_noord_def |>
  filter(
    measure %in% c(demos, eigendom, economie, groen),
    spatial_type == 'buurten',
    spatial_code != 'NOVERIG',
    spatial_code != 'NFOCUS'
  ) |>
  ...
  mutate(
    orpubgroen_p = orpubgroen / orpubland * 100,
    orpubgroen_inw = orpubgroen / bevtotaal * 1000,
    opp_boomkroon_publiek_p = opp_boomkroon_publiek_ha /
      orpubland *
      100,
    orbuurtgroen_p = orbuurtgroen / orpubland * 100,
    orntrgroen_p = orntrgroen / orpubland * 100,
...
som_df2 <- som_df |>
  left_join(vars_kernmerken, by = "measure")
...
BBGA_data_noord_def_wide <- BBGA_data_noord_def2 |>
  ...
  mutate(
    `naam indicator` = case_when(
      kernindicator_noord == TRUE ~ glue::glue(
        "{`naam indicator`} (kernindicator)"
      ),
      TRUE ~ `naam indicator`
    )
  ) |>
...
list_noord <- split(
  BBGA_data_noord_def_wide,
  f = BBGA_data_noord_def_wide$thema_noord_eenmeting
) |>
...
saveWorkbook(
  wb_noord,
  glue::glue(
    "04 tabellen/02 tabellen noord/tabel alle data { naam_focusgebied } { datum_vandaag }.xlsx"
  ),
  overwrite = T
)
saveWorkbook(
  wb_noord,
  glue::glue(
    "04 tabellen/05 tabellen website focusgebieden/tabel alle data { naam_focusgebied } { datum_vandaag }.xlsx"
  ),
  overwrite = T
)
...
openxlsx::write.xlsx(
  BBGA_data_noord_def2,
  glue::glue(
    "04 tabellen/02 tabellen noord/tabel alle data long { naam_focusgebied } { datum_vandaag }.xlsx"
  ),
  overwrite = T
)
