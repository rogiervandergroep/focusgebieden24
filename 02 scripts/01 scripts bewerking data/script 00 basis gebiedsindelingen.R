...
my_spatial_type <- function(type, jaar, datum) {
  pad <- "https://onderzoek.amsterdam.nl/static/datavisualisatie-onderzoek-en-statistiek/geo/amsterdam/"

  st_read(glue::glue("{pad}/{jaar}/{type}-{jaar}-geo.json")) |>
    st_drop_geometry() |>
    add_column(spatial_date = datum, spatial_type = type) |>
    rename(spatial_code = code, spatial_name = naam) |>
    select(spatial_code, spatial_name, spatial_date, spatial_type)
}

my_spatial_wg_type <- function(type, datum) {
  pad <- "https://onderzoek.amsterdam.nl/static/datavisualisatie-onderzoek-en-statistiek/geo/winkelgebieden/2024/winkelgebieden-2024-geo.json"
