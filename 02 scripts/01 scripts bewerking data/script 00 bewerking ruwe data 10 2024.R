...
df_thuiswonend <- read.csv2(
  "00 ruwe data/ruw input 10 2024/data_thuiswonend.csv"
) |>
  add_column(tweedeling = "totaal")
write.xlsx(
  df_thuiswonend,
  "00 ruwe data/niet in bbga/data_thuiswonend_24_def.xlsx"
)
...
df_abm_totaal <- openxlsx::read.xlsx(
  "00 ruwe data/ruw input 10 2024/Burgermonitor tbv BBGA.xlsx"
) |>
  add_column(tweedeling = 'totaal')
...
write.xlsx(
  df_abm_df,
  "00 ruwe data/niet in bbga/data_burgermonitor_24_def.xlsx"
)
# variabelen staan in inputlijst -
...
df_flex_werkend <- read.xlsx(
  "00 ruwe data/ruw input 10 2024/20240912_aandeelflex.xlsx"
) |>
  add_column(tweedeling = 'totaal') |>
  select(
    spatial_code,
...
write.xlsx(
  df_flex_werkend,
  "00 ruwe data/niet in bbga/data_polisadministratie_24_def.xlsx"
)
...
df_aandeel_ondern_sd <- read.xlsx(
  "00 ruwe data/ruw input 10 2024/20240912_aandeel_zelfstandigen_gewogen.xlsx",
  sheet = 1
) |>
  rename(
    'temporal_date' = 'Jaar',
...
df_aandeel_ondern_ams <- df_aandeel_ondern_sd |>
  ...
  mutate(
    value = Zelfstandigen /
      (Flexibel.dienstverband + Vast.dienstverband + Zelfstandigen)
  ) |>
...
#NB: deze tabel vervangt data_rebebb (alleen data beschikbaar op SD en Gebiedsniveau)
df_aandeel_ondern_geb <- read.xlsx(
  "00 ruwe data/ruw input 10 2024/20240912_aandeel_zelfstandigen_gewogen.xlsx",
  sheet = 2
) |>
  rename(
...
df_aandeel_ondern <- bind_rows(
  df_aandeel_ondern_sd,
  df_aandeel_ondern_geb,
  df_aandeel_ondern_ams
) |>
...
write.xlsx(
  df_aandeel_ondern,
  "00 ruwe data/niet in bbga/data_aandeelondernemers_19_23_def.xlsx"
)
...
## JEUGDHULP aanpassen naar BBGA indicatoren jeugdvoorzienningen 0 18 jaar :
############################################################################################################
...
df_secm_neet <- df_secm |>
  ...
  mutate(measure = 'NEET_1826_P', value = 100 - value)

write.xlsx(
  bind_rows(df_secm, df_secm_neet),
  "00 ruwe data/niet in bbga/data_secm_24_def.xlsx"
)

df_startkwal <- read.xlsx(
  "00 ruwe data/ruw input 10 2024/mpzo_startkwalificaties_ses.xlsx"
) |>
  rename(
    value = waarde,
    tweedeling = tweedeling_sd
...
write.xlsx(
  df_startkwal,
  "00 ruwe data/niet in bbga/data_startkwal_ses_24_def.xlsx"
)
...
df_buit <- read.xlsx(
  "00 ruwe data/ruw input 10 2024/jongeren_leeftijdsgroep_geboorteplaats_buurt_wijk.xlsx",
  sheet = 'T5'
) |>
  ...
  mutate(
    measure = case_when(
      LEEFTIJDSGROEP == '18 t/m 27 jaar' &
        GEBOORTEPLAATS == 'Amsterdam' ~ 'geb_ams_18_27_p',
      LEEFTIJDSGROEP == '12 t/m 17 jaar' &
        GEBOORTEPLAATS == 'Amsterdam' ~ 'geb_ams_12_17_p',

      LEEFTIJDSGROEP == '18 t/m 27 jaar' &
        GEBOORTEPLAATS == 'Buiten Amsterdam' ~ 'geb_nl_18_27_p',
      LEEFTIJDSGROEP == '12 t/m 17 jaar' &
        GEBOORTEPLAATS == 'Buiten Amsterdam' ~ 'geb_nl_12_17_p',

      LEEFTIJDSGROEP == '18 t/m 27 jaar' &
        GEBOORTEPLAATS == 'Buitenland' ~ 'geb_bui_18_27_p',
      LEEFTIJDSGROEP == '12 t/m 17 jaar' &
        GEBOORTEPLAATS == 'Buitenland' ~ 'geb_bui_12_17_p'
    )
  )
...
SES_WOA <- read.csv2(
  "00 ruwe data/ruw input 10 2024/Sociaal_economische_status_21102024_165251.csv"
) |>
  purrr::set_names(c(
    "temporal_date",
    "spatial_name",
...
write.xlsx(
  SES_WOA_wijk,
  "00 ruwe data/ruw input 10 2024/SES_gewogen.xlsx"
)
...
write.xlsx(
  bind_rows(woning_tot, nieuwbouw),
  "00 ruwe data/niet in bbga/data_won_mutaties.xlsx"
)
...
write.xlsx(
  bind_rows(schuld_sd, schuld_ams),
  "00 ruwe data/niet in bbga/data_prob_schulden.xlsx"
)

# veiligheidsindexen toegevoegd eind november 2024
veiligheidsindex <- openxlsx::read.xlsx(
  "00 ruwe data/ruw input 10 2024/data_veiligheidsindex_stadsdelen.xlsx",
  sheet = 'index_basis'
)

veiligheidsindex_long <- veiligheidsindex |>
  pivot_longer(
    cols = c(v_gercrim_i:v_vermijd_i),
    names_to = 'measure',
    values_to = 'value'
  ) |>
  add_column(spatial_date = '20220324')
...
write.xlsx(
  veiligheidsindex_long,
  "00 ruwe data/niet in bbga/data_veiligheidsindex.xlsx"
)
...
df_zittend <- bind_rows(
  openxlsx::read.xlsx(
    "00 ruwe data/ruw input 10 2024/zittende_en_nieuwe_bewoners_in_stadsdeel.xlsx",
    sheet = 'buurt'
  ) |>
  ...
  mutate(
    bew_nieuw = (aantal_nieuwe_bewoners_in_stadsdeel / totaal_18_plussers) *
      100,
