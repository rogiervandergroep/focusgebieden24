...
data_def3 <- data_def2 |>
  mutate(
    value = case_when(
      str_ends(measure, "_p") ~ round(value),
      str_ends(measure, "_r") ~ round(value, 1),
      measure == 'bhstart_oph' ~ round(value, 1),
      TRUE ~ round(value)
    )
  )
...
BBGA_data_zo_buurt <- data_def3 |>
  ...
  mutate(
    thema_zuidoost_label = str_replace(
      thema_zuidoost_label,
      'SD3 Openbare ruimte en mobiliteit',
      'SD3 ruimte en mob.'
    )
  ) |>
  mutate(
...
naam_monitor <- 'Outcomemonitor Masterplan Zuidoost tussenmeting 2026'
...
saveWorkbook(
  wb_zuidoost,
  glue::glue(
    "04 tabellen/01 tabellen zuidoost/tabel alle data { naam_focusgebied } { datum_vandaag }.xlsx"
  ),
  overwrite = T
)
saveWorkbook(
...
openxlsx::write.xlsx(
  BBGA_data_zo,
  glue::glue(
    "04 tabellen/01 tabellen zuidoost/tabel alle data long { naam_focusgebied } { datum_vandaag }.xlsx"
  )
)
