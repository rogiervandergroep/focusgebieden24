...
BBGA_data_nw <- data_def2 |>
  ...
  left_join(
    aanpak_nw_buurten_wf,
    by = 'spatial_code'
  ) |>
  mutate(
    focusbuurt = case_when(is.na(focusbuurt) ~ FALSE, TRUE ~ focusbuurt),
    kernindicator_nw = case_when(
      is.na(kernindicator_nw) ~ 'basis',
      TRUE ~ kernindicator_nw
    ),
    thema_nw_kleur = case_when(
      is.na(thema_nw_kleur) ~ 'basis',
      TRUE ~ thema_nw_kleur
    ),
    thema_nw_label = case_when(
      is.na(thema_nw_label) ~ 'basis',
      TRUE ~ thema_nw_label
    )
  ) |>
  arrange(temporal_date, measure)
...
BBGA_data_nw_wide <- BBGA_data_nw_def |>
  ...
  arrange(kernindicator_nw, measure) |>
...
naam_monitor <- 'Outcomemonitor Samen Nieuw-West tussenmeting 2026'
...
saveWorkbook(
  wb_nw,
  glue::glue(
    "04 tabellen/03 tabellen nieuwwest/tabel alle data { naam_focusgebied } { datum_vandaag }.xlsx"
  ),
  overwrite = T
)
saveWorkbook(
...
openxlsx::write.xlsx(
  BBGA_data_nw_def,
  glue::glue(
    "04 tabellen/03 tabellen nieuwwest/tabel alle data long { naam_focusgebied } { datum_vandaag }.xlsx"
  )
)
